...
class EmailSender:
    ...
    def __init__(self, account: dict = None):
        if account is None:
            ...
            account = ACCOUNT_ALIYAN

        self.account = account
        self.smtp_host = account.get("smtp_host", SMTP_HOST)
        self.smtp_port = account.get("smtp_port", SMTP_PORT)
        self.smtp_user = account.get("email", SMTP_USER)
        self.smtp_password = account.get("password", SMTP_PASSWORD)
        self.imap_host = account.get("imap_host", IMAP_HOST)
        self.imap_port = account.get("imap_port", IMAP_PORT)
        self.sender_name = account.get("name", SENDER_NAME)
        self.sender_email = account.get("email", SENDER_EMAIL)
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.info(f"Connected to SMTP server {self.smtp_host}:{self.smtp_port} as {self.smtp_user}")
            return True
        except smtplib.SMTPAuthenticationError as e:
            logger.error(f"SMTP authentication failed for {self.smtp_user}: {e}")
            logger.error("Check your SMTP password in .env file")
            return False
        except Exception as e:
            logger.error(f"SMTP connection failed for {self.smtp_user}: {e}")
            return False
    ...
    def _save_to_sent_folder(self, msg):
        ...
        try:
            imap = imaplib.IMAP4_SSL(self.imap_host, self.imap_port, timeout=10)
            ...
            logger.debug(f"Could not copy to Sent folder for {self.smtp_user}: {e}")
        ...
        try:
            ...
            if lead_id:
                ...
                email_id = db.log_email_sent(
                    ...
                    from_email=self.sender_email, to_email=to_email,
                )
    ...
    def send_test(self, to_email: str = None) -> dict:
        ...
        subject = f"Mahad Impex Email System — Test ({self.sender_name})"
        sig = self.account.get("signature", "")
        body = (
...
def check_for_replies_and_bounces(account: dict = None):
    """
    Check IMAP inbox for replies, bounces, and unsubscribe requests.
    If account is provided, checks that account. Otherwise checks all configured ACCOUNTS.
    """
    from config import ACCOUNTS
    accounts_to_check = [account] if account else ACCOUNTS
    total_result = {"replies": 0, "bounces": 0, "unsubscribes": 0}

    for acc in accounts_to_check:
        imap_host = acc.get("imap_host", IMAP_HOST)
        imap_port = acc.get("imap_port", IMAP_PORT)
        user = acc.get("email", SMTP_USER)
        pwd = acc.get("password", SMTP_PASSWORD)
        ...
        try:
            ...
            for num in msg_nums[-50:]:  # Process last 50 unread
                ...
                if "unsubscribe" in subject or "unsubscribe" in body_lower:
                    ...
                    total_result["unsubscribes"] += 1
                    logger.info(f"Unsubscribe processed ({user}): {from_addr}")
                    continue
                ...
                if any(indicator in subject for indicator in
                    ...
                    total_result["bounces"] += 1
                    logger.info(f"Bounce detected ({user}): {from_addr}")
                    continue
                ...
                if lead:
                    ...
                    total_result["replies"] += 1
                    logger.info(f"Reply received ({user}) from: {from_addr}")

            mail.logout()
        except imaplib.IMAP4.error as e:
            logger.error(f"IMAP error for {user}: {e}")
        except Exception as e:
            logger.error(f"Error checking inbox for {user}: {e}")

    logger.info(f"Inbox check total: {total_result}")
    return total_result
